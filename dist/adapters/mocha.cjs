...
__export(mocha_exports, {
  createCoverageAfterAll: () => createCoverageAfterAll
});
...
function ratioColour(covered, total) {
  if (total === 0) return (0, import_colorette.gray)("0/0");
  const pct = covered / total * 100;
  const str = `${covered}/${total}`;
  if (pct >= 100) return (0, import_colorette.green)(str);
  if (pct >= 50) return (0, import_colorette.yellow)(str);
  return (0, import_colorette.red)(str);
}
function printReport(coverage, opts = {}) {
  ...
  const tclStr = coverage.tcl >= 4 ? (0, import_colorette.green)(String(coverage.tcl)) : coverage.tcl >= 1 ? (0, import_colorette.yellow)(String(coverage.tcl)) : (0, import_colorette.red)(String(coverage.tcl));
  console.log();
  console.log((0, import_colorette.bold)((0, import_colorette.cyan)("--- OpenAPI coverage ---")));
  console.log((0, import_colorette.bold)("TCL:"), tclStr, (0, import_colorette.dim)(`(${tclLabel})`));
  console.log((0, import_colorette.dim)("  Paths:               "), ratioColour(coverage.path.covered, coverage.path.total));
  console.log((0, import_colorette.dim)("  Operations:          "), ratioColour(coverage.operation.covered, coverage.operation.total));
  console.log((0, import_colorette.dim)("  Status codes:        "), ratioColour(coverage.statusCode.covered, coverage.statusCode.total));
  console.log((0, import_colorette.dim)("  Parameters:          "), ratioColour(coverage.parameter.covered, coverage.parameter.total));
  console.log((0, import_colorette.dim)("  Parameter values:    "), ratioColour(coverage.parameterValue.covered, coverage.parameterValue.total));
  console.log((0, import_colorette.dim)("  Response body props:  "), ratioColour(coverage.responseBodyProperties.covered, coverage.responseBodyProperties.total));
  console.log((0, import_colorette.dim)("  Response asserted:    "), ratioColour(coverage.responseAsserted.covered, coverage.responseAsserted.total));
  console.log((0, import_colorette.dim)("  Input content-type:   "), ratioColour(coverage.inputContentType.covered, coverage.inputContentType.total));
  console.log((0, import_colorette.dim)("  Output content-type: "), ratioColour(coverage.outputContentType.covered, coverage.outputContentType.total));
  console.log((0, import_colorette.dim)("  Status code class:   "), ratioColour(coverage.statusCodeClass.covered, coverage.statusCodeClass.total));
  console.log((0, import_colorette.dim)("-------------------------------------------"));
  console.log();
  if (verbose) {
    const showMissing = (label, metric) => {
      if (metric.missing.length === 0) return;
      console.log((0, import_colorette.bold)((0, import_colorette.yellow)(label)));
      metric.missing.slice(0, maxMissingItems).forEach((item) => console.log((0, import_colorette.dim)("  ") + (0, import_colorette.gray)(item)));
      if (metric.missing.length > maxMissingItems) {
        console.log((0, import_colorette.dim)(`  ... and ${metric.missing.length - maxMissingItems} more`));
      }
      console.log();
    };
...
function writeJsonReport(coverage, filepath) {
  ...
  import_node_fs.default.writeFileSync(filepath, JSON.stringify(coverage, null, 2), "utf8");
}
...
`;
  import_node_fs2.default.writeFileSync(filepath, html, "utf8");
}

// src/adapters/mocha.ts
function createCoverageAfterAll(options) {
  return function openApiCoverageAfterAll() {
    ...
    if (consoleOpts !== false) {
      ...
      const maxMissingItems = typeof consoleOpts === "object" ? consoleOpts.maxMissingItems : void 0;
    ...
    if (minTcl != null && !Number.isNaN(minTcl) && coverage.tcl < minTcl) {
      throw new Error(
        `OpenAPI coverage TCL ${coverage.tcl} is below required minimum ${minTcl}. Set OPENAPI_COVERAGE_MIN_TCL to override (e.g. 5 to allow lower).`
      );
    }
  };
}
